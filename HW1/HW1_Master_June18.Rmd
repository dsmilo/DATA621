...
This is a model where all predictors are used, and the data is not processed at all, except to replace NA values with the medians of the respective column; predictors are sequentially removed until all have p values below a 0.05 threshold.

The predictors with high numbers of NA values - TEAM_BATTING_HBP (2085 NA values) and TEAM_BASERUN_CS (772 NAs) have no significance - likely because their values are so uniform, giving them little ability to explain the response.
...
for (i in 1:16) {
  #ifelse(is.na(data_no_index[,i]), median(data_no_index[,i], na.rm = TRUE), data_no_index[,i])
  data_no_index[,i][is.na(data_no_index[,i])] <-  median(data_no_index[,i], na.rm = TRUE)
}
df_new <- data_no_index
summary(df_new)
