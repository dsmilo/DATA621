...
```{r}
dfraw <- read.csv(url("https://raw.githubusercontent.com/dsmilo/DATA621/master/HW1/data/moneyball-training-data.csv"))
dfremove <- subset(dfraw, TEAM_BATTING_SO == 0 | INDEX == 1347 | TEAM_BATTING_BB == 0 | TEAM_PITCHING_SO > 1535 | TEAM_PITCHING_SO < 308 | TEAM_BATTING_SO < 308 | TEAM_BATTING_SO > 1535)$INDEX #Identify rows for removal, have zero strikeouts or bases stolen
...
summary(fit4)
fit5 <- update(fit4, .~. - TEAM_FIELDING_DP) #Removal of TEAM_FIELDING_DP (which also has a coefficient opposite the expected sign) decreases F-stat, 153.2 to 170.4
summary(fit5)
