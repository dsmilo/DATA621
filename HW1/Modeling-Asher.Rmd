...
TEAM_PITCHING_SO == 0 | There's a gradient of strikeouts until 181, and then it goes directly to zero, which seems suspect
...
TEAM_PITCHING_SO | Outside of recognized records [link](http://www.baseball-almanac.com/recbooks/rb_strike2.shtml) - may not be relevant if the data is not from the MLB
...
I then created a linear regression, and iterated by removing predictors with low significance, until the f-statistic of the regression stopped increasing with the removal of predictors. [The f-stats mentioned in the comments may have changed]

```{r}
dfraw <- read.csv(url("https://raw.githubusercontent.com/dsmilo/DATA621/master/HW1/data/moneyball-training-data.csv"))
dfremove <- subset(dfraw, TEAM_BATTING_SO == 0 | TEAM_PITCHING_SO == 0 | INDEX == 1347 | TEAM_BATTING_BB == 0 | TEAM_PITCHING_SO > 1535 | TEAM_PITCHING_SO < 308)$INDEX #Identify rows for removal, have zero strikeouts or bases stolen
...
summary(fit1)
fit2 <- update(fit1, .~. -TEAM_PITCHING_BB) #Removal of TEAM_PITCHING_BB increases F-stat, 123.5 to 134.8
summary(fit2)
fit3 <- update(fit2, .~. - TEAM_BATTING_2B) #Removal of TEAM_BATTING_2B increases F-stat, 134.8 to 147.1
summary(fit3)
fit4 <- update(fit3, .~. - TEAM_PITCHING_H) #Removal of TEAM_PITCHING_H increases F-stat, 147.1 to 152.7
summary(fit4)



fit5 <- update(fit4, .~. - TEAM_FIELDING_DP) #Removal of TEAM_FIELDING_DP decreases F-stat, 152.7 to 125.8
summary(fit5)


#Potential stepwise regression - has no effect
step4 <- step(fit4)
summary(step4)
