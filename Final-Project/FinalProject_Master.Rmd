...
##Abstract
Regression techniques are used to predict loan approval status of customers of a fictional bank given a number of predictors.  Techniques utilized include logistic regression, ridge regression, the Adaboost algorithm, and neural networks.  The optimal model created yields an accuracy of 0.8056.
...
```{r fig.width=4, fig.height=2,echo=FALSE, eval = TRUE}
...
##Methodology
  The dataset has 614 rows (each representing a customer) and 13 predictor variables variables. Furthermore, there is 1 identification variable, and 1 response variables: `Loan_Status`, a binary categorical variable representing whether each customer has been approved for a loan given their credentials.  The class of variables read in from the dataset is presented below:

```{r data-class, echo = FALSE}
train_var = train
train_var$Property_Area = as.factor(train_var$Property_Area)
var_class <- data.frame(Class = rep(NA, ncol(train_var) - 1), Levels = rep(NA, ncol(train_var) - 1), stringsAsFactors = FALSE, check.names = FALSE, row.names = names(train_var)[-1])
names(var_class) = c("Class", "Levels/Range")

for(i in 2:ncol(train_var)) {
...
cont_vars <- train_var %>% dplyr::select(c(ApplicantIncome, CoapplicantIncome, LoanAmount, Loan_Amount_Term, TotalIncome, LTI))
...
#Discrete Variables
poisson_vars <- train_var %>% dplyr::select(-c(ApplicantIncome, CoapplicantIncome, LoanAmount, Loan_Amount_Term, TotalIncome, LTI, Property_Area))
...
Given the variables in the data, two new features were created: `Total Income` and `LTI` (Loan to Income Ratio). The `Urban` and `SemiUrban` variables were created from the `Property_Type` variable, and the `Property_Type` varible was excluded from further analyses. As all other data were numeric, or binary factors converted to numeric, these data worked well for logistic regression.

Logistic regression is a very common method of classifying data (Thomas, 2000). In logistic regression, a linear regression is used where the dependent variable is a non-linear function of the probability of the event occuring. In this project, the purpose of the logistic regression method is to classify cases as either "Approved for loan" or "Denied". The general logistic regression model can be represented as: 
...
```{r fig.width=6, fig.height=4,echo=FALSE, eval = TRUE}
#This mathematical model can be expressed visually, and is depicted in the following figure:
#![](http://neuralnetworksanddeeplearning.com/images/tikz10.png)
