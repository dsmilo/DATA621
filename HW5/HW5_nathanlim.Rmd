...
for (j in 1:k) {
  ...
  model1 <- glm(TARGET ~ VolatileAcidity+ LabelAppeal + AcidIndex + STARS, data = wine_new[folds != j, ], family =  poisson )
  model2 = glm(TARGET ~ VolatileAcidity+LabelAppeal + AcidIndex + STARS, data = wine_new[folds != j, ], family =  gaussian )
