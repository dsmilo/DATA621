...
Histograms each variable in the data set after casewise deleteion is below:
...
Examinging all of the variables, there do not appear to be any values far outside of the ranges, values that are nonsensical (negative cases purchased, for example), or need to be removed before moving forward. The combination of variables, or ratios of existing predictors did not seem to yield any effective results. Log or square root transformations may help to improve models using some of the slightly skewed predictors.  
...
Seven of the predictor coeffcients are significant under a reasonable $\alpha$ of 0.05, and one more, `pH` being just over the threshold. The coefficients themselves are fairly small, with most of the more significant predictors having greater slopes than those which are not. Eight of the coefficients have negative slopes, while the other six are positive. Those with positive coefficients, such as STARS and Label Appeal, make sense as greater ratings and more appealing label aesthetics would entice buyers to purchase more wine.  
...
```{r}
model1 <- (glm(TARGET ~ VolatileAcidity + Chlorides + FreeSulfurDioxide + TotalSulfurDioxide + LabelAppeal + AcidIndex + STARS, family="poisson", data=train))
pander(summary(model1))
#AIC: 31701
```  

Of the seven predictors, four have positive coefficients, three have negative slopes. Increases in acidity and chlorides seem to have a negative effect on the number of cases purchased; this may have to do with any wines with more extreme values in these predictors to demand a specific pallette.  All of the coefficients are significant under a p-value of 0.05, with the predictors most correlated to `TARGET` being the most significant. The reduced model using the recommended predictors from Mallow's $C_p$ only returns a very slight reduction in AIC.
...
```{r}
model3 <- (glm.nb(TARGET ~ VolatileAcidity + Chlorides + FreeSulfurDioxide + TotalSulfurDioxide + Alcohol + AcidIndex + LabelAppeal + STARS, data=train))
...
The characteristics and performance of the six multiple linear regression models from the previous section are compared below:

|Model #| Type | # of Predictors|AIC|
|---|---|---|---|
|1|Poisson|7|31701|
|2|Poisson|3|31739|
|3|Negative Binomial|8|31705|
...
for (j in 1:k) {
  ...
  model1 <- glm(TARGET ~ VolatileAcidity + Chlorides + FreeSulfurDioxide
                 + TotalSulfurDioxide + LabelAppeal + AcidIndex + STARS, family="poisson", data=train[folds != j, ])
...
names(all.cv.error) = c("Poisson Model 1", "Poisson Model 2", "NB Model 3", "NB Model 4", "MLR Model 5", "MLR Model 6")
#all.cv.error
...
predicted_cases_stars = round(predicted_cases, 0)
```

The predicted ratings are converted to even star values by rounding to the nearest integer.  A table of the proportion of ratings at each star level is presented for both the test and training datasets.

```{r, echo = FALSE}
table_test <- table(predicted_cases_stars) / length(predicted_cases_stars)
...
The full sets of predictions -- both raw predicted values and even star values -- for the evaluation dataset are available in Appendix A.
...
appendixA[, 3] = predicted_cases_stars[1:1667]
appendixA[, 4] = test$IN[1669:3335]
appendixA[, 5] = predicted_cases[1669:3335]
appendixA[, 6] = predicted_cases_stars[1669:3335]
appendixA <- rbind(appendixA, c(test$IN[1668], predicted_cases[1668], predicted_cases_stars[1688], rep(NA, 3)))

names(appendixA) = rep(c("Index", "Value", "Stars"), 2)
