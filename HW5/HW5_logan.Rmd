...
```{r bic_selection}
regfit.full=regsubsets(TARGET ~., data=train)
...
for (j in 1:k) {
  ...
  model6 <- glm(TARGET ~ VolatileAcidity + FreeSulfurDioxide + TotalSulfurDioxide + 
    ...
    for (i in 1:10) {
      ...
      cv.errors3[j, i] = mean((train$TARGET[folds_amt == j] - pred3) ^ 2, na.rm = TRUE)
       
      pred4 = predict(model4, f, id = i) 
      cv.errors4[j, i] = mean((train$TARGET[folds_amt == j] - pred4) ^ 2, na.rm = TRUE)
       
      pred5 = predict(model5, f, id = i) 
      cv.errors5[j, i] = mean((train$TARGET[folds_amt == j] - pred5) ^ 2, na.rm = TRUE)      
...
predicted_cases_bin = ifelse(predicted_cases > 0.5, 1, 0)
```

#####Predicted Targets for Test Data
```{r, echo = FALSE}
pander(table(predicted_cases_bin))
```

#####Targets for Training Data
```{r, echo = FALSE}
training_cases = train$TARGET
pander(table(training_cases))
```
...
```{r appendix-a}
appendixA = data.frame(matrix(NA, nrow = 3335, ncol = 2))
appendixA[, 1] = c(1:3335)
appendixA[, 2] = predicted_cases_bin[1:3335]

names(appendixA) = c("Index", "Predicted Value", "Index", "Predicted Value")
