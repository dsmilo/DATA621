...
test = read.csv("https://github.com/dsmilo/DATA621/raw/master/HW4/Data/insurance-evaluation-data.csv")
test2 = read.csv("https://github.com/dsmilo/DATA621/raw/master/HW4/Data/insurance-evaluation-data.csv")
...
```{r logistic-full-model, echo = FALSE}
...
```{r logistic-reduced-model, echo = FALSE}
...
```{r logistic-best-subsets, echo = FALSE}
...
plotROC(model = bestsubset19, ndata = train_flag[, -1], gtruth = train$TARGET_FLAG)
model8 = bestsubset19
```
...
pander(table(train_flag$TARGET_FLAG, train_flag$target_bestsub))
```
...
##Model Comparison
The characteristics and performance of the five multiple linear regression models from the previous section are compared below:

|Model #|# of Predictors|AIC|
|---|---|---|
|1|38|44678|
|2|38|5235.6|
|3|1|5196.2|
...
The characteristics and performance of the three binary logistic regression models from the previous section are compared below:
...
#Remove all $ signs and commas, i.e. from INCOME, HOME_VAL, BLUEBOOK, OLDCLAIM
test$INCOME <- extract_numeric(test$INCOME)
test$HOME_VAL <- extract_numeric(test$HOME_VAL)
test$BLUEBOOK <- extract_numeric(test$BLUEBOOK)
test$OLDCLAIM <- extract_numeric(test$OLDCLAIM)
...
#EDUCATION
test$HSDropout <- ifelse(test$EDUCATION=="<High School", 1, 0)
test$HS <- ifelse(test$EDUCATION=="z_High School", 1, 0)
test$Bachelors <- ifelse(test$EDUCATION=="Bachelors", 1, 0)
test$Masters <- ifelse(test$EDUCATION=="Masters", 1, 0)
test$PhD <- ifelse(test$EDUCATION=="PhD", 1, 0)

#CAR_TYPE
test$Minivan <- ifelse(test$CAR_TYPE=="Minivan", 1, 0)
test$Panel_Truck <- ifelse(test$CAR_TYPE=="Panel Truck", 1, 0)
test$Pickup <- ifelse(test$CAR_TYPE=="Pickup", 1, 0)
test$Sports_Car <- ifelse(test$CAR_TYPE=="Sports Car", 1, 0)
test$Van <- ifelse(test$CAR_TYPE=="Van", 1, 0)
test$SUV <- ifelse(test$CAR_TYPE=="z_SUV", 1, 0)

#JOB
test$Blank_Job <- ifelse(test$JOB == "", 1, 0)
test$Professional <- ifelse(test$JOB == "Professional", 1, 0)
test$Blue_Collar <- ifelse(test$JOB == "z_Blue Collar", 1, 0)
test$Clerical <- ifelse(test$JOB == "Clerical", 1, 0)
test$Doctor <- ifelse(test$JOB == "Doctor", 1, 0)
test$Lawyer <- ifelse(test$JOB == "Lawyer", 1, 0)
test$Manager <- ifelse(test$JOB == "Manager", 1, 0)
test$Home_Maker <- ifelse(test$JOB == "Home Maker", 1, 0)
test$Student <- ifelse(test$JOB == "Student", 1, 0)

test <- test %>% dplyr::select(-c(INDEX,EDUCATION,CAR_TYPE,JOB))
```
...
k = 10
set.seed(1306)
folds = sample(1:k, nrow(train_flag), replace = TRUE)
...
for (j in 1:k) {
  ...
  model1 <- glm(TARGET_AMT ~., data = train_amt[folds_amt != j, ]) 
  ...
  model8 = glm(TARGET_FLAG ~ KIDSDRIV + INCOME + PARENT1 + HOME_VAL + MSTATUS + TRAVTIME + CAR_USE + BLUEBOOK + TIF + OLDCLAIM + CLM_FREQ + REVOKED + MVR_PTS + URBANICITY + HSDropout + HS + Panel_Truck + Van + PhD, data = train_flag[folds != j, ], family = binomial(link='logit'))
    ...
    for (i in 1:10) {
      f = train_amt[folds_amt == j, ]
      f = f[complete.cases(f),]
      pred1 = predict(model1, f, id = i)
      cv.errors1[j, i] = mean((train_amt$TARGET_AMT[folds_amt == j] - pred1) ^ 2, na.rm = TRUE)
    
      pred2 = predict(model2, f, id = i) %>% exp()
      cv.errors2[j, i] = mean((train_amt$TARGET_AMT[folds_amt == j] - pred2) ^ 2, na.rm = TRUE)
...
The model using only the significant predictors as determined by AIC (Model 1) was selected as the best model for prediction of `TARGET_AMT` in this data set. While the AIC value of this model was not the highest of the models tested, its mean cross-validation error indicates that it is has the best predictive value for unseen data. Additionally, it is a parsimonious model, and the simplicity lends itself to easier understanding of the model by other users.
...
The linear model is applied to a test dataset containing response variables for 2141 cases.
...
predicted_amt <- predict(model1, test)
predicted_amt2 = predicted_amt
predicted_amt2[] = 0

predicted_flag = predict(model8, test, type = "response")
...
A table of the proportion of target flags is presented below for both the test and training datasets.

##### Classifications for Target Flag  {-}
```{r, echo = FALSE}
table_predicted <- table(predicted_flag_bin) / length(predicted_flag_bin)
table_train <- table(train_flag$TARGET_FLAG) / length(train_flag$TARGET_FLAG)
table_flag <- rbind(table_predicted, table_train)
row.names(table_flag) <- c("Test", "Train")
pander(table_flag)
```

##### Distribution of Target Amount {-}
Histograms of the distribution of target amounts are prepared for the test and training datasets.

```{r, echo = FALSE}
par(mfrow = c(1,2))
hist(predicted_amt2, xlim = range(0, 10000), main = "Predicted Test Distribution")
hist(train$TARGET_AMT, main = "Training Distribution")
```

An additional pair of histograms is created which considers only customers with flags equal to one; this allows visibility into the distribution of target amounts for customers who had accidents.

```{r}
par(mfrow = c(1,2))
had_accidents <- predicted_flag_bin == 1
predicted_amt_accidents <- predicted_amt2[had_accidents]
hist(predicted_amt_accidents, xlim = range(0, 10000), main = "Predicted Test Distribution")
hist(train_amt$TARGET_AMT, main = "Training Distribution")
par(mfrow = c(1,1))
```

Similar to the training dataset, the predictions for the test data set predictions are weighted toward zero. It is important to note that Model 1 did not achieve the highest AIC on the training data but, given its cross-validation performance, we believe that Model 1 will produce the greatest generalizability of the models explored here for prediction of `TARGET_AMT`. Likewise, given the Cross Validation results, we believe that Logistic Model 3 would provide the best generalizability for the classifcation task.
...
appendixA[,1] = test2$INDEX
appendixA[,2] = predicted_flag
appendixA[,3] = predicted_flag_bin
...
# Log transform -- addition of 0.01 to correct for 0 values yieling NaN
cors_flag_log <- as.vector(cor(train$TARGET_FLAG, log(train[,1:ncol(train)] + 0.01)))
cors_amt_log  <- as.vector(cor(train$TARGET_AMT, log(train[,1:ncol(train)] + 0.01)))
