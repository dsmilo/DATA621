...
summary(model5)                           # Summary of the linear regression model
...
#### Model 6: All predictors used

Like Model 1, this is formed using best subsets regression with BIC as a criterion for choosing the number of predictors; however, with this model, predictors are removed sequentially until all predictors are statistically significant (p < 0.05). This leads to the removal of age and median home values as predictors.
```{r, echo = FALSE, fig.height=2}
model6 <- glm(target ~., data = train, family = binomial(link='logit'))
summary(model6)                           # Summary of the linear regression model

train_df$predicted_model6 <- predict(model6, train_df, type = 'response')
train_df$target_model6 <- ifelse(train_df$predicted_model6 > 0.5, 1, 0)
roc_model6 <- pROC::roc(factor(target) ~ predicted_model6, data=train_df)

plot(roc_model6, col="red")
confusionMatrix(train_df$target, train_df$target_model6, positive = "1")
```
